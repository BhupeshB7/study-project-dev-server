...
import { ServiceConfigModel } from "../models/serviceConfig.model.js";
...
export const submitServiceRequest = async (payload, user) => {
  ...
  if (serviceConfig) {
    if (
      serviceConfig.eligibleRoles.length > 0 &&
      !serviceConfig.eligibleRoles.includes(user.role)
    ) {
      throw Object.assign(
        new Error("You are not eligible to apply for this service"),
        { statusCode: 403 },
      );
    }
    ...
    const activeCount = await ServiceRequestModel.countDocuments({
      ...
      status: {
        $in: [
          ...
          ServiceRequestStatus.AWAITING_DOCUMENTS,
        ],
      },
    });

    if (activeCount >= serviceConfig.maxActiveRequestsPerUser) {
      throw Object.assign(
        new Error(
          `You already have ${activeCount} active request(s) for this service. Maximum allowed is ${serviceConfig.maxActiveRequestsPerUser}.`,
        ),
        { statusCode: 400 },
      );
    }
  ...
  try {
    ...
    reqLogger.info(
      `Service request submitted: ${serviceRequest.requestNumber}`,
    );
